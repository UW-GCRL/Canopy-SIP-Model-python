"""
Canopy-SIP Model — Web Interface (Streamlit).
...
with st.sidebar:
    ...
    if JAX_AVAILABLE:
        backend = st.radio(
            "⚡ Compute Backend",
            ["NumPy/SciPy", "JAX (JIT + GPU)"],
            help="JAX backend provides JIT compilation and GPU acceleration."
        )
    else:
        backend = "NumPy/SciPy"
        st.info("Install `jax` for GPU acceleration & autodiff.")
...
if st.button("▶️ Run Simulation", type="primary", use_container_width=True):
    with st.spinner("Running BRF simulation..."):
        ...
        if backend == "JAX (JIT + GPU)":
            from canopy_sip.jax_backend import run_simulation as run_sim_jax
            result = run_sim_jax(
                SZA=SZA, SAA=SAA,
